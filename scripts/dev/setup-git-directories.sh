...
echo "📁 In ~/code: $(git config user.email 2>/dev/null || echo '[work email] (work - default)')"

cd ~/.dotfiles
echo "📁 In ~/.dotfiles: $(git config user.email 2>/dev/null || echo 'satanin@gmail.com (personal - override)')"

cd ~
echo "📁 In ~/ (home): $(git config user.email 2>/dev/null || echo '[work email] (work - default)')"

echo ""
echo "✅ Configuration Summary:"
echo "   📧 Default email: [work email] (work)"
